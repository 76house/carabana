...
$(document).ready(function()
{
    ...
    $("body").mousemove(function(event) {
        ...
        $('#carabana-logo #tube').animate({ left: (xTube + 385) + 'px', top: yTube + 'px'}, options);

        xLandscape = Math.min(x * 0.015, 10);
        yLandscape = Math.min(y * 0.03, 20);
        $('#carabana-logo #city').animate({ left: (xLandscape + 220) + 'px', top: (yLandscape + 1) + 'px'}, options);
        $('#carabana-logo #village').animate({ left: (xLandscape + 458) + 'px', top: (yLandscape + 82) + 'px'}, options);

        xSun = Math.min(x * 0.01, 7);
        ySun = Math.min(y * 0.02, 13);
        $('#carabana-logo #sun').animate({ left: (xSun + 515) + 'px', top: (ySun - 6) + 'px'}, options);
